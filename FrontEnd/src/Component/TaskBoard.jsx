...
const TaskBoard = ({ selectedProjectId, onBack, globalSearch }) => {
    ...
    try {
      ...
      sortedTasks.forEach((task) => {
        ...
        if (!colMap[task.status]) {
          colMap[task.status] = {
            name: task.status.replace(/_/g, " ").toUpperCase(),
            items: [],
          };
        }
        if (!task.isArchived) {
    ...
    try {
      ...
      if (selectedProjectId && projects.length > 0) {
        ...
        if (activeProj) {
          ...
          if (activeProj.owner)
            projectMembers.push({ ...activeProj.owner, role: "Owner" });
  ...
  const renameList = async (columnId) => {
    ...
    try {
      ...
      const newCols = { ...columns };
      const items = newCols[columnId].items.map((it) => ({
        ...it,
        status: newStatus,
      }));
      delete newCols[columnId];
  ...
  const onDragEnd = async (result) => {
    ...
    if (source.droppableId === destination.droppableId) {
      ...
      setColumns({
        ...
        [source.droppableId]: {
          ...sourceCol,
          items: sourceItems.map((it, i) => ({ ...it, position: i })),
        },
        [destination.droppableId]: {
          ...destCol,
          items: destItems.map((it, i) => ({ ...it, position: i })),
        },
      });
      try {
        await axios.put(
            ...
            <div>
              ...
              <div className="flex items-center gap-4 text-[10px] font-black text-slate-500 tracking-[0.2em] uppercase opacity-70">
                <span className="flex items-center gap-1">
                  <Users className="w-3.5 h-3.5" /> {users.length} members
                </span>
                <span className="flex items-center gap-1">
                  <ClockIcon className="w-3.5 h-3.5" /> {projectStats.total}{" "}
                  active tasks
                </span>
              </div>
            </div>
                          ...
                          {(provided) => (
                            ...
                            >
                              <div
                                ...
                                className="p-5 pb-2 flex items-center justify-between cursor-grab active:cursor-grabbing"
                              >
                                ...
                                {activeMenuCol === columnId && (
                                  <div className="absolute top-14 right-4 w-40 bg-white border border-slate-200 rounded-xl shadow-2xl z-50 py-2">
                                    <button
                                      ...
                                      className="w-full text-left px-4 py-2 text-[10px] font-black text-rose-500 hover:bg-rose-50 uppercase tracking-widest"
                                    >
                                      Delete List
                                    </button>
                                  </div>
                                            ...
                                            >
                                              ...
                                              <div className="p-4 space-y-3">
                                                ...
                                                <div className="flex items-center gap-3 text-slate-500 pt-1">
                                                  {item.dueDate && (
                                                    <div
                                                      className={`flex items-center gap-1 text-[9px] font-black px-2 py-0.5 rounded ${new Date(item.dueDate) < new Date() && item.status !== "completed" ? "bg-rose-50 text-rose-600" : "bg-slate-100"}`}
                                                    >
                                                  ...
                                                  <div className="flex items-center gap-3 opacity-60">
                                                    ...
                                                    {total > 0 && (
                                                      <div
                                                        className={`flex items-center gap-1 ${completed === total ? "text-emerald-500 opacity-100" : ""}`}
                                                      >
                                  ...
                                  <div className="bg-white p-3 rounded-xl shadow-lg border border-slate-200 animate-in zoom-in-95 duration-200">
                                    ...
                                    <div className="flex items-center gap-2">
                                      <button
                                        ...
                                        className="flex-1 bg-indigo-600 text-white py-2 rounded-lg font-black text-[10px] uppercase tracking-widest"
                                      >
                                        Add
                                      </button>
                                      <button
                      ...
                      <div className="w-80 shrink-0 bg-slate-50 border border-slate-200 rounded-2xl p-4 animate-in zoom-in-95 duration-200 h-fit">
                        ...
                        <div className="flex gap-2">
                          ...
                          <button
                            ...
                            className="px-3 py-2 text-slate-400 hover:bg-slate-200 rounded-lg"
                          >
                            <X className="w-4 h-4" />
                          </button>
                        </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
                {[
                  ...
                  >
                    ...
                    <p
                      className={`text-3xl font-black ${stat.color} tracking-tight`}
                    >
                      {stat.value}
                    </p>
                  </div>
              ...
              <div className="bg-white border border-slate-200 rounded-3xl p-10 shadow-sm">
                ...
                <div className="space-y-10">
                  {[
                    {
                      label: "To do",
                      count: projectStats.todo,
                      color: "bg-slate-400",
                    },
                    {
                      label: "Doing",
                      count: projectStats.doing,
                      color: "bg-indigo-600",
                    },
                    {
                      label: "Done",
                    ...
                    <div key={item.label} className="space-y-4">
                      ...
                      <div className="h-3 w-full bg-slate-100 rounded-full overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${projectStats.total ? (item.count / projectStats.total) * 100 : 0}%`,
                          }}
                        />
                      </div>
                    </div>
            ...
            <div className="p-12 animate-in fade-in duration-500 bg-white">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {users.map((user) => (
                  <div
                    key={user._id}
                    className="bg-white border border-slate-200 rounded-[2rem] p-8 flex items-center justify-between group hover:shadow-lg transition-all relative overflow-hidden"
                  >
                    ...
                    <div className="flex items-center gap-6 relative z-10">
                      ...
                      <div className="overflow-hidden">
                        <div className="flex items-center gap-3 mb-1">
                          <h4 className="text-lg font-black text-slate-900 truncate tracking-tight uppercase group-hover:text-indigo-600 transition-all">
                            {user.name}
                          </h4>
                          {user.role && (
                            <span
                              className={`px-2 py-0.5 rounded-full text-[8px] font-black uppercase tracking-widest ${user.role === "Owner" ? "bg-amber-100 text-amber-600" : "bg-indigo-100 text-indigo-600"}`}
                            >
                              {user.role}
                            </span>
                          )}
                        </div>
                        <p className="text-xs font-bold text-slate-500 opacity-60 truncate">
                          {user.email}
                        </p>
                        <p className="text-[10px] font-black text-indigo-600 uppercase tracking-widest mt-3 opacity-0 group-hover:opacity-100 transition-all">
                          View Profile
                        </p>
                      </div>
                    </div>
