...
export default function TaskModal({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        assignedTo:
          typeof editedTask.assignedTo === "object"
            ? editedTask.assignedTo?._id
            : editedTask.assignedTo,
        project:
          typeof editedTask.project === "object"
            ? editedTask.project?._id
            : editedTask.project,
  ...
  const priorityCfg =
    PRIORITY_CONFIG[editedTask.priority] || PRIORITY_CONFIG.Medium;
      ...
      >
        ...
        <header className="px-8 py-5 border-b border-slate-100 flex items-center gap-6 shrink-0 bg-white">
          <div className="flex items-center gap-3 shrink-0">
            ...
            <div
              className={`text-[10px] font-bold px-2.5 py-1 rounded-xl border ${statusCfg.bg} ${statusCfg.color} border-current/10`}
            >
              {statusCfg.label}
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar flex flex-col lg:flex-row bg-main/50">
              ...
              <div className="max-w-3xl mx-auto space-y-8 px-4">
                <div className="relative pl-8 space-y-8 before:absolute before:left-[15px] before:top-2 before:bottom-2 before:w-[2px] before:bg-slate-100">
                  {(editedTask.activity || task.activity || []).length > 0 ? (
                    [...(editedTask.activity || task.activity || [])]
                      .reverse()
                      .map((log, i) => (
                        <div key={i} className="relative">
                          ...
                          <div className="space-y-1">
                            ...
                            <time className="text-[9px] font-bold text-slate-300 uppercase block tracking-widest">
              ...
              <div className="max-w-3xl mx-auto space-y-8">
                <div className="space-y-4">
                  {(editedTask.comments || []).map((c) => (
                    <div
                      key={c._id}
                      className="flex gap-4 p-5 bg-card border border-base rounded-2xl shadow-sm"
                    >
          ...
          <aside className="w-full lg:w-80 bg-card border-l border-base flex flex-col overflow-visible">
              ...
              <div className="grid grid-cols-1 gap-2">
                {Object.entries(STATUS_CONFIG).map(([s, cfg]) => (
                  ...
                  >
                    <span>{cfg.label}</span>
                    {editedTask.status === s && (
                      <CheckCircleIconSolidInner className="w-4 h-4 text-emerald-400" />
                    )}
                  </button>
                ))}
              </div>
            ...
            <div className="space-y-4">
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest px-1 text-indigo-600">
                Assignment
              </p>
              <div className="relative">
                <div className="relative">
                  <input
                    ...
                    value={
                      assigneeSearch ||
                      (typeof editedTask.assignedTo === "object"
                        ? editedTask.assignedTo?.name
                        : editedTask.pendingAssigneeEmail
                          ? `Invited: ${editedTask.pendingAssigneeEmail}`
                          : editedTask.assignedTo || "")
                    }
                    onChange={(e) => {
                        ...
                        !/^\w+([.-]?\w+)*@\w+([.-]?\w+)*(\.\w{2,3})+$/.test(
                          ...
                          <div className="px-4 py-10 text-center space-y-2">
                            ...
                            <p className="text-[9px] text-slate-300 font-medium">
                              Type a valid email to invite anyone.
                            </p>
                          </div>
                        )}
                    </div>
            ...
            <div className="space-y-4">
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest px-1 text-emerald-600">
        ...
        <footer className="px-8 py-5 border-t border-base flex justify-between items-center shrink-0 bg-card">
          ...
          <div className="flex gap-3">
            <button
              className="px-6 py-2.5 text-slate-400 hover:text-slate-900 font-bold text-xs uppercase transition-all"
              onClick={onClose}
            >
              Discard
            </button>
