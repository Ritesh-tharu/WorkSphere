      ...
      >
                ...
                <div className="grid grid-cols-1 gap-2">
                    {Object.entries(STATUS_CONFIG).map(([s, cfg]) => (
                        <button
                            ...
                            className={`flex items-center justify-between p-3 rounded-xl text-xs font-bold border transition-all ${editedTask.status === s ? `bg-slate-900 text-white border-slate-900 shadow-md` : 'text-slate-500 bg-slate-50 border-slate-200 hover:border-slate-300'}`}
                        >
                            <span>{cfg.label}</span>
        ...
        <footer className="px-8 py-5 border-t border-slate-100 flex justify-end gap-3 shrink-0 bg-white">
          ...
          >
            ...
            {loading ? "Syncing…" : saved ? "Synced!" : "Save changes"}
          </button>
        </footer>
      </div>
    </div>
