...
const ProjectManager = ({ initialSelectedId, globalSearch }) => {
    ...
    return (
      <div className="flex-1 p-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[1, 2, 3, 4].map((i) => (
          <div
            key={i}
            className="h-40 bg-slate-100 dark:bg-slate-800 animate-pulse rounded-2xl border border-slate-200"
          />
        ))}
      </div>
    );
  ...
  return (
    <div className="flex-1 bg-main min-h-full p-8 lg:p-8 overflow-y-auto no-scrollbar animate-in fade-in duration-700  ">
          ...
          <div className="space-y-4 pt-1">
            ...
            <div className="flex items-center gap-6 text-sm font-bold text-secondary uppercase tracking-widest opacity-60">
              <span className="flex items-center gap-2">
                <LayoutGrid className="w-4 h-4" /> {filteredProjects.length}{" "}
                {filteredProjects.length === 1 ? "board" : "boards"}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-12">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4 text-primary">
              ...
              <h2 className="text-xl font-black tracking-tighter uppercase text-[12px] tracking-[0.25em]">
                Your boards
              </h2>
            </div>
            {globalSearch && (
            ...
            {filteredProjects.map((p) => (
              ...
              >
                ...
                <div className="px-1 flex items-center justify-between">
                  <h3
                    className="text-sm font-black text-primary tracking-tight truncate group-hover:text-indigo-600 transition-colors uppercase pr-2"
                    onClick={() => setActiveBoardId(p._id)}
                  >
                    {p.name}
                  </h3>
                    ...
                    <>
                      ...
                      <div className="absolute right-0 top-10 w-48 bg-white border border-base rounded-xl shadow-2xl z-50 py-2 animate-in zoom-in-95 duration-200">
                        <button
                          ...
                          className="w-full text-left px-4 py-2 text-[11px] font-black text-slate-700 hover:bg-slate-50 uppercase tracking-widest flex items-center gap-3"
                        >
                          <Pencil className="w-3.5 h-3.5" /> Edit Board
                        </button>
                        <button
                          onClick={() => handleDeleteProject(p._id)}
                          className="w-full text-left px-4 py-2 text-[11px] font-black text-rose-500 hover:bg-rose-50 uppercase tracking-widest flex items-center gap-3 border-t border-base mt-2 pt-4"
                        >
                          <X className="w-3.5 h-3.5" /> Delete Board
                        </button>
                      </div>
                    </>
                  )}
      ...
      {activeSubTab === "members" && (
        <div className="flex flex-col items-center justify-center py-20 text-center opacity-40">
          ...
          <p className="text-sm font-bold">
            Manage your workspace collaborators and permissions.
          </p>
        </div>
      )}
      ...
      {activeSubTab === "settings" && (
        <div className="flex flex-col items-center justify-center py-20 text-center opacity-40">
          ...
          <p className="text-sm font-bold">
            Configure workspace identity, visibility and security tags.
          </p>
        </div>
      )}
      ...
      {showCreateModal && (
        ...
        >
          <div
            className="bg-[#1d2125] text-white w-full max-w-3xl max-h-[90vh] rounded-xl border border-[#ffffff10] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200"
          ...
          >
            ...
            >
              ...
              <div className="relative z-10 w-full flex justify-end">
                <button
                  onClick={closeModal}
                  className="p-1 hover:bg-white/20 rounded text-white group"
                >
                ...
                <div className="grid grid-cols-3 sm:grid-cols-4 xl:grid-cols-5 gap-3">
                  {THEMES.map((t, i) => (
                    <button
                      ...
                      className={`h-10 rounded-xl transition-all relative overflow-hidden bg-center bg-cover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 ${newProject.color === t.color ? "ring-2 ring-indigo-500 scale-105 z-10 shadow-lg" : "hover:scale-105 opacity-80 hover:opacity-100"}`}
                      style={{
                        backgroundColor: t.color,
                        backgroundImage:
                          t.type === "image" ? `url(${t.url})` : "none",
                      }}
                    >
                      {newProject.color === t.color && (
                        <div className="absolute inset-0 flex items-center justify-center bg-black/10">
                          <CheckCircleIconSolidInner className="w-4 h-4 text-white" />
                        </div>
                      )}
                    </button>
                  ))}
              ...
              <form onSubmit={handleSaveProject} className="space-y-5">
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    <ChevronDown
                      className={`w-4 h-4 text-slate-400 transition-transform ${showVisibilityDropdown ? "rotate-180" : ""}`}
                    />
                  </button>
